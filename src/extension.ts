...
let restoreDebuggersCommand = vscode.commands.registerCommand('extension.restoreDebuggers', async () => {
    const files = await vscode.workspace.findFiles('**/*.{js,ts}', '**/node_modules/**');
    ...
    for (const file of files) {
        ...
        if (changesMade) {
            await vscode.workspace.applyEdit(edit);
            await document.save();
            vscode.window.showInformationMessage(`Debugger statements have been restored in file: ${file.fsPath}`);
        } else {
            vscode.window.showInformationMessage(`No debugger statements found in file: ${file.fsPath}`);
        }
    }

    vscode.window.showInformationMessage('Finished restoring debugger statements!');
});
